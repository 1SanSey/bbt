...
import 'package:bbt/utils/pull_to_refresh_flutter3/src/indicator/classic_header.dart';
import 'package:bbt/utils/pull_to_refresh_flutter3/src/indicator/material_classic_header.dart';
import 'package:bbt/utils/pull_to_refresh_flutter3/src/internals/refresh_indicator.dart';
import 'package:bbt/utils/pull_to_refresh_flutter3/src/internals/refresh_physics.dart';
import 'package:bbt/utils/pull_to_refresh_flutter3/src/internals/sliver_refresh.dart';
...
class SmartRefresherState extends State<SmartRefresher> {
  ...
  List<Widget>? _buildSliversByChild(
    ...
    if (widget.enablePullDown || widget.enableTwoLevel) {
      slivers?.insert(
        0,
        widget.header ?? configuration?.headerBuilder?.call() ?? defaultHeader,
      );
    }
    //insert header or footer
    if (widget.enablePullUp) {
      slivers?.add(widget.footer ?? configuration?.footerBuilder?.call() ?? defaultFooter);
    }
  ...
  ScrollPhysics _getScrollPhysics(RefreshConfiguration? conf, ScrollPhysics physics) {
    ...
    return _physics = RefreshPhysics(
      ...
      enableScrollWhenRefCompleted: conf?.enableScrollWhenRefCompleted ?? false,
    ...
    if (childView is! Scrollable) {
      ...
      body = Scrollable(
        ...
        viewportBuilder: (context, offset) {
          final Viewport viewport = childView.viewportBuilder(context, offset) as Viewport;
          ...
          //insert header or footer
          if (widget.enablePullUp) {
            viewport.children.add(widget.footer ?? conf?.footerBuilder?.call() ?? defaultFooter);
          }
  ...
  bool _ifNeedUpdatePhysics() {
    final conf = RefreshConfiguration.of(context);
    ...
    if (conf.topHitBoundary != _physics!.topHitBoundary ||
        ...
        _physics!.enableScrollWhenRefCompleted != conf.enableScrollWhenRefCompleted) {
      return true;
    }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.builder != null) {
      body = widget.builder!(
        context,
        _getScrollPhysics(configuration, const AlwaysScrollableScrollPhysics()) as RefreshPhysics,
      );
    } else {
      final slivers = _buildSliversByChild(context, widget.child, configuration);
...
class RefreshController {
  ...
  Future<void>? requestRefresh({
    ...
    if (needMove) {
      return Future.delayed(const Duration(milliseconds: 50)).whenComplete(() async {
      ...
      await Future.value().whenComplete(() {
        headerMode!.value = RefreshStatus.refreshing;
      });
    }
...
class RefreshConfiguration extends InheritedWidget {
  ...
  final bool enableScrollWhenRefCompleted;
  ...
  const RefreshConfiguration({
    ...
    this.enableScrollWhenRefCompleted = false,
  ...
  RefreshConfiguration.copyAncestor({
    ...
    bool? hideFooterWhenNotFull,
        ...
        enableScrollWhenRefCompleted = enableScrollWhenRefreshCompleted ??
            RefreshConfiguration.of(context)!.enableScrollWhenRefCompleted,
  ...
  bool updateShouldNotify(RefreshConfiguration oldWidget) {
    return skipCanRefresh != oldWidget.skipCanRefresh ||
        ...
        enableScrollWhenRefCompleted != oldWidget.enableScrollWhenRefCompleted ||
