...
class _MaterialClassicHeaderState extends RefreshIndicatorState<MaterialClassicHeader>
  ...
  void initState() {
    ...
    _positionFactor = _positionController
        .drive(Tween<Offset>(begin: const Offset(0, -1), end: Offset(0, widget.height / 44.0)));
    super.initState();
  }
...
class _WaterDropMaterialHeaderState extends _MaterialClassicHeaderState {
  ...
  void initState() {
    ...
    _positionFactor = _positionController
        .drive(Tween<Offset>(begin: const Offset(0, -0.5), end: const Offset(0, 1.5)));
  }
...
class _BezierPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (value < 0.5) {
      final Path path = Path()
        ..moveTo(0, 0)
        ..quadraticBezierTo(middleX, value * 70.0, size.width, 0);
      canvas.drawPath(path, paint);
    } else if (value <= 1.0) {
      final Path path = Path();
      ...
      path
        ...
        ..quadraticBezierTo(middleX - 40.0 * (value - 0.5), 40.0 - value * 40.0, size.width, 0)
        ..moveTo(size.width, 0)
        ..lineTo(0, 0);
      canvas.drawPath(path, paint);
    } else {
      final Path path = Path()
        ..moveTo(0, 0)
        ..conicTo(middleX, 60.0 * (1.5 - value), size.width, 0, 5);
      canvas.drawPath(path, paint);
    }
  }
