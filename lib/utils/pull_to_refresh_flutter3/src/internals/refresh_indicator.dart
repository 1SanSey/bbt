...
abstract class RefreshIndicatorState<T extends RefreshIndicator> extends State<T>
  ...
  @override
  Future<void> _dispatchModeByOffset(double offset) async {
    if (mode == RefreshStatus.twoLeveling) {
      if (_position!.pixels > configuration!.closeTwoLevelDistance &&
        ...
        await refresher!.controller.twoLevelComplete();
    ...
    if ((configuration!.enableBallisticRefresh && activity!.velocity < 0.0) ||
      ...
      if (refresher!.enablePullDown && offset >= configuration!.headerTriggerDistance) {
        if (!configuration!.skipCanRefresh) {
          ...
          await readyToRefresh().whenComplete(() {
            if (!mounted) return;
  ...
  Future<void> _handleModeChange() async {
    ...
    if (mode == RefreshStatus.completed || mode == RefreshStatus.failed) {
      await endRefresh().whenComplete(() {
        if (!mounted) return;
        floating = false;
        if (mode == RefreshStatus.completed || mode == RefreshStatus.failed) {
          refresherState!.setCanDrag(configuration!.enableScrollWhenRefCompleted);
        }
        update();
        /*
      ...
      if (!floating) {
        floating = true;
        await readyToRefresh();
      }
      if (configuration!.enableRefreshVibrate) {
        await HapticFeedback.vibrate();
      }
      if (refresher!.onRefresh != null) refresher?.onRefresh?.call();
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        _position!
            .animateTo(0, duration: const Duration(milliseconds: 500), curve: Curves.linear)
            .whenComplete(() {
          mode = RefreshStatus.twoLeveling;
        });
        if (refresher!.onTwoLevel != null) refresher?.onTwoLevel?.call(true);
      });
    } else if (mode == RefreshStatus.twoLevelClosing) {
      floating = false;
      refresherState!.setCanDrag(false);
      update();
      if (refresher!.onTwoLevel != null) refresher?.onTwoLevel?.call(false);
...
abstract class LoadIndicatorState<T extends LoadIndicator> extends State<T>
  ...
  @override
  double _calculateScrollOffset() {
    return math.max(_position!.pixels - _position!.maxScrollExtent, 0);
  }

  Future<void> enterLoading() async {
    setState(() {
      floating = true;
    });
    _enableLoading = false;
    await readyToLoad().whenComplete(() {
      if (!mounted) {
        return;
      }
  ...
  void _dispatchModeByOffset(double offset) {
    ...
    if (activity is DragScrollActivity) {
      mode = _checkIfCanLoading() ? LoadStatus.canLoading : _lastMode;
    }
...
mixin RefreshProcessor {
  ...
  void onOffsetChange(double _) {}

  /// mode change callback
  void onModeChange(RefreshStatus? _) {}
...
mixin LoadingProcessor {
  ...
  void onModeChange(LoadStatus? _) {}
