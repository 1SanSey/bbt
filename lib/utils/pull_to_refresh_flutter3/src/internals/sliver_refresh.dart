// ignore_for_file: prefer_asserts_with_message, avoid_setters_without_getters
    ...
    if (refreshStyle == RefreshStyle.front) {
      ...
      super.layout(
        (constraints as SliverConstraints)
            .copyWith(overlap: math.min(0, renderViewport.offset.pixels)),
        parentUsesSize: true,
      );
    } else {
      ...
      switch (refreshStyle) {
        case RefreshStyle.follow:
          geometry = SliverGeometry(
            ...
            layoutExtent:
                math.min(needPaintExtent, math.max(layoutExtent - constraints.scrollOffset, 0)),
          );
...
class RenderSliverLoading extends RenderSliverSingleBoxAdapter {
  ...
  double? computePaintOrigin(double? layoutExtent, bool reverse, bool follow) {
    if (follow) {
      ...
      return reverse
          ? math.max(constraints.viewportMainAxisExtent - constraints.precedingScrollExtent, 0) +
              layoutExtent!
          : math.max(constraints.viewportMainAxisExtent - constraints.precedingScrollExtent, 0);
    }
  }
  ...
  void performLayout() {
    ...
    bool active;
    active = hideWhenNotFull! && mode != LoadStatus.noMore ? _computeIfFull(constraints) : true;
    if (active) {
      child!.layout(constraints.asBoxConstraints(), parentUsesSize: true);
    } else {
      child!.layout(constraints.asBoxConstraints(maxExtent: 0, minExtent: 0), parentUsesSize: true);
    }
    final double childExtent =
        constraints.axis == Axis.vertical ? child!.size.height : child!.size.width;
    final double paintedChildSize = calculatePaintOffset(constraints, from: 0, to: childExtent);
    final double cacheExtent = calculateCacheOffset(constraints, from: 0, to: childExtent);
...
class RenderSliverRefreshBody extends RenderSliverSingleBoxAdapter {
  ...
  void performLayout() {
    ...
    if (childExtent == 1111111) {
      child!.layout(
        constraints.asBoxConstraints(maxExtent: constraints.viewportMainAxisExtent),
        parentUsesSize: true,
      );
    }
    switch (constraints.axis) {
